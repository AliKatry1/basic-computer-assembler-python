class Assembler(object):
    ...
    def __first_pass(self) -> None:
        ...
        lc = 0
        for i in self.__asm:
            if self.__islabel(i[0]):
                self.__address_symbol_table[i[0][:3]] = self.__format2bin(str(lc), "dec", 12)
                lc += 1
            elif i[0] == "org":
                lc = int(i[1], 16)
            elif i[0] == "end":
    ...
    def __second_pass(self) -> None:
        ...
        for i in self.__asm:
            if i[0] == "org":
                ...
                return
            elif len(i) > 2 and i[1] == "dec":
                self.__bin[self.__format2bin(str(lc), "dec", 12)] = self.__format2bin(str(i[2]), "dec", 16)
                lc += 1
            elif len(i) > 2 and i[1] == "hex":
                self.__bin[self.__format2bin(str(lc), "dec", 12)] = self.__format2bin(str(i[2]), "hex", 16)
                lc += 1
            instruction = i[1] if self.__islabel(i[0]) else i[0]
            if instruction in self.__mri_table:
                opcode = str(self.__mri_table[instruction])
                address = str(self.__address_symbol_table[i[1]])
                self.__bin[self.__format2bin(str(lc), "dec", 12)] = str(0) + opcode + address
                lc += 1
            elif instruction in self.__rri_table:
                self.__bin[self.__format2bin(str(lc), "dec", 12)] = str(self.__rri_table[instruction])
                lc += 1
            elif instruction in self.__ioi_table:
